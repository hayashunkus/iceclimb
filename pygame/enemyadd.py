...
while running and cap.isOpened():
    ...
    if not game_over:
        ...
        left_flick_detected = False # ★リセット
        right_flick_detected = False # ★リセット
        
        left_cursor_pos[:] = [-100, -100]  # 検出されなければ画面外へ
        right_cursor_pos[:] = [-100, -100] # 検出されなければ画面外へ
        left_flick_pos[:] = [-100, -100] # ★リセット
        right_flick_pos[:] = [-100, -100] # ★リセット
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                if handedness.classification[0].label == 'Left':
                    ...
                    # ★デコピン速度計算
                    left_middle_tip_y[0] = left_middle_tip_y[1] # 1フレーム前のY座標を[0]へ
                    left_middle_tip_y[1] = flick_pos_y         # 現フレームのY座標を[1]へ
                    flick_velocity = left_middle_tip_y[0] - left_middle_tip_y[1] # 上方向への移動量
        ...
        if left_flick_detected:
            pygame.draw.circle(screen, BLUE, left_flick_pos, cursor_radius + 10, 5) # 少し大きい円
