...
MAX_PULL_METERS = 2.0
# ★ ゴール判定用の設定
GOAL_THRESHOLD_METERS = 97.0 # ★ 修正: 100.0 -> 97.0 (最大98.0mより手前)
GOAL_ZONE_METERS = 3.0 # ★ 修正: 1.0 -> 3.0 (最後の3mはホールド無し)
...
try:
    for i in range(1, 6):
        ...
        dancer_images.append(pygame.image.load(img_path).convert_alpha())
...
try:
    ...
    GOAL_ZONE_PIXELS = GOAL_ZONE_METERS * PIXELS_PER_METER # 3m * 360 = 1080 pixels
...
is_near_goal = False # ★追加: 97m以上に到達したか
...
while running:
    ...
    if game_won:
        ...
        if dancer_images:
            ...
            # 画面中央に配置
            img_rect = current_dancer_image.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2))
        ...
        # --- ★エネミーの更新と当たり判定 ---
        if not is_near_goal: # 97m未満の場合のみ
        ...
        # ★ 97m地点の判定
        is_near_goal = height_climbed >= GOAL_THRESHOLD_METERS

        if is_near_goal and goal_background_image:
            # ★ 97m以上なら、ホールドやエネミーを隠すためにゴール背景を上書き
            screen.blit(goal_background_image, (0, 0))
        else:
            # ★ 97m未満なら通常通り描画
            if hold_image:
        ...
        if height_climbed >= 98.0: # 物理的な頂上
             height_text_str = f"Height: {TOTAL_CLIMB_METERS:.1f} m GOAL!"
